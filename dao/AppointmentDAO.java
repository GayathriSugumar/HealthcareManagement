...
public class AppointmentDAO {
	...
	public boolean addAppointment(AppointmentEntity appointment) {
		...
		try {
			String sqlStatement = "insert into appoinment(userId, fullName, gender, age, appointmentDate, mobile, illness, doctorId, address, status) values(?,?,?,?,?,?,?,?,?,?)"; 
			...
			prepareStatement.setString(6, appointment.getMobile());
			prepareStatement.setString(7, appointment.getIllness());
			prepareStatement.setInt(8, appointment.getDoctorId());
			prepareStatement.setString(9, appointment.getAddress());
			prepareStatement.setString(10, appointment.getStatus());
	...
	public List<AppointmentEntity> getAllAppointmentsByUserId(int userId){
		...
		try {
			...
			while (result.next()) {
				...
				appointment.setMobile(result.getString(7));
				appointment.setIllness(result.getString(8));
				appointment.setDoctorId(result.getInt(9));
				appointment.setAddress(result.getString(10));
				appointment.setStatus(result.getString(11));
