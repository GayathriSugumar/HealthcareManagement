...
//		DoctorDAO doctorDao=new DoctorDAO(ConnectionProvider.getconnection());
		...
		try {
			...
			PreparedStatement prepareStatement = connection.prepareStatement("update userDetails set password=? where email=?");
			...
			if (rowCount > 0) {
				...
				dispatcher = req.getRequestDispatcher("user_login.jsp");
			} else {
				req.setAttribute("status", "resetFailed");
				dispatcher = req.getRequestDispatcher("UserChangePassword.jsp");
			}
